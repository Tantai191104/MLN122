...
export const articleService = {
  ...
  async getArticlesByCategory(category: string, page = 1, pageSize = 10): Promise<ArticleListResponse> {
    const response = await axiosInstance.get<ArticleListResponse>(`/articles/category/${category}`, {
  ...
  async getRelatedArticles(articleId: string, limit = 3): Promise<Article[]> {
    const response = await axiosInstance.get<Article[]>(`/articles/${articleId}/related`, {
      params: { limit },
    })
    return response.data
  },
